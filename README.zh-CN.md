...
- 图组恢复流程只在版面证据、页面坐标和精确 Markdown 范围一致时，才从本地 PDF 恢复完整图表。图组渲染在后台 Worker 中完成，拼图过程不会阻塞阅读界面和 Zotero 其他操作，同一页有多个图组时只渲染一次。转换完成后即可开始阅读：尚未拼好的图组显示动画占位（lucide 图片图标加旋转 loader），每拼好一个就就地替换，慢的图不会拖住其余内容。关闭阅读标签页不会停止正在进行的恢复，任务会继续并在完成后更新缓存。所有图表处理完成前，导出与保存快照保持禁用。生成的本地 PNG 会保留 2x2、4x4、缺角、跨行和不规则排列。只有裁切成功后，图内 OCR 才会从阅读流移除；图注仍可选择、搜索和翻译。证据不足的图组会保留原图片和文字，并显示非阻断提示。MinerU 恢复需要受支持的详细布局数据；Mistral 的坐标原点和旋转方向尚未独立验证，因此当前生产适配器会保留原图片和 OCR 文字。
...
- 保留可靠的页码和区域映射，使正文、公式、表格和图片可以跳回 PDF 来源。阅读时在段旁显示当前段落对应的 PDF 原貌裁切，点击可跳到 PDF 中的对应位置。可在阅读工具栏或设置中关闭缩略图。
...
4. 使用阅读器工具栏打开排版弹出层设置字号、字体、行距、栏宽和对齐，进入专注模式、切换阅读模式或翻译。论文含 GitHub 仓库时，工具栏会显示 GitHub 图标。校对、快照、导出和引用图谱在溢出菜单中。`Escape` 退出专注模式；查找和排版在专注模式中仍可用。
