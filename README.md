...
- 在 Markdown 正文和渲染后的表格、图题等内容中显示 Zotero PDF 高亮与下划线标注，
  ...
  脚注上标和商标上标格式差异；带评论的标注会在起始左上角显示笔记标记，悬停、点击或键盘
  聚焦标注时会显示对应评论和 PDF 页码。
...
4. 在只读视图中选择和复制文本，使用 `Ctrl+F` 或 `Cmd+F` 搜索，并通过目录、引用预览
   和图表预览浏览文档。已有的 Zotero PDF 高亮和下划线会使用原标注颜色显示；悬停、
   点击或用键盘聚焦划词内容可查看评论与页码。
...
Mktero Markdown 缓存。每次转换或从缓存打开文档时都会重新读取标注。
