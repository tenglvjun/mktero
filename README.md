...
- 对 MinerU `content_list.json` 中能与 Markdown 唯一匹配的段落、公式、图片和表格保留
  PDF 页码与区域坐标。悬停对应 Markdown 块或用键盘聚焦时会显示外部链接图标，点击后
  打开原 PDF 并高亮对应区域；一个 Markdown 段落由多个 PDF 块合并时会保留全部来源，
  第一版跳转到其中第一个位置。短文本、重复文本或其他不确定匹配不会显示跳转按钮。
- 对具有可靠来源映射的选区或结构块，可复制带论文标题、PDF 物理页码和 Zotero 回链的
  Markdown。正文以引用块复制，公式和 GFM 表格保留安全结构，图片只复制图题而不暴露
  本地缓存路径；普通复制行为保持不变。
