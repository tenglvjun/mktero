...
- 可从 Markdown 标签页重新解析当前 PDF；操作提示会明确说明 PDF 将再次上传，并可能消耗
  转换服务额度。重新解析期间保留当前内容，新结果失败时继续显示原结果。
...
- 在普通 Markdown 正文中拖动划词后显示紧凑操作条，可选择颜色立即保存 Markdown 高亮，
  ...
  不会仅为同步而创建 PDF 标签页。同步成功后，在 Markdown 或 PDF 中改色、修改笔记和
  删除都会同步，已打开的 Markdown 标签页会自动刷新 PDF 端的标注变化。
...
4. 在只读视图中选择和复制文本，使用 `Ctrl+F` 或 `Cmd+F` 搜索，并通过目录、引用预览
   ...
   主动打开该 PDF 时同步，Mktero 不会仅为同步而创建 PDF 标签页。
5. 需要忽略缓存并重新解析时，点击 Markdown 标签页中的刷新按钮。该操作会再次上传 PDF，
   并可能消耗转换服务额度。
