...
- 识别数字引用、上标引用以及数字或字母作者单位；作者单位编号与等贡献、通讯作者等符号
  ...
  论文引用；MinerU 省略 Introduction 标题时，也会从作者署名之后继续识别正文上标引用，
  并区分短英文单词后的引用与 `m²`、`cm²` 等指数。悬停时预览文献内容，点击后跳转并临时
  高亮目标。
...
- 在普通 Markdown 正文中拖动划词后显示紧凑操作条，可选择颜色立即保存 Markdown 高亮，
  ...
  原因并允许手动重试，重试不会重新上传或解析 PDF。非阻塞提醒以顶部 toast 显示，
  几秒后自动收起，不会挤压正文。同步成功后，在 Markdown 或 PDF 中
...
4. 在只读视图中选择和复制文本，使用 `Ctrl+F` 或 `Cmd+F` 搜索，并通过目录、引用预览
   ...
   到操作条途中经过其他标注不会打断当前选区操作。Markdown 高亮会立即保存并显示，并通过
   本地 PDF 文本坐标索引创建 Zotero 标注，无需预先打开 PDF。索引尚在生成时，同步会等待
   同一个后台任务；无法可靠定位时会保留本地高亮并显示失败原因。右侧笔记栏中的
...
或 Zotero 会停止当前的本地查询和 PDF 索引任务，但不会远程取消已完成上传的 MinerU 任务。
以后再次主动打开相同 PDF 时，Mktero 会在任务记录有效期内继续查询结果，并复用已经写入的
本地 PDF 文本索引。
...
上传。每次转换或从缓存打开文档时都会重新读取标注；评论新增或修改、改色和删除操作都会
直接保存到当前 Zotero 文库。
...
`mktero-annotations/v1` 目录中。Mktero 会使用转换时并行生成或按需加载的本地 PDF 索引
尝试写入 Zotero 标注，不要求打开 PDF，也不会为此新建 PDF 标签页。同步成功后由 Zotero
负责本地保存与同步，并移除临时的 Mktero 本地记录；内容不会发送给 MinerU。只有原文能
在 PDF 中可靠定位时才会同步，失败的记录会保留在原目录中，不会猜测位置。已经打开的
Zotero PDF 阅读器只作为本地索引不可用时的兼容回退。
...
- 从 Markdown 新建标注时会先立即保存本地高亮。MinerU 转换和 PDF.js 索引并行执行，
  Markdown 阅读不会等待索引；同步需要坐标时会等待正在运行的同一个索引任务，但不会自动
  打开前台或后台 PDF 标签页。具有可靠单页来源映射的选区会在对应物理页内要求唯一匹配，
  因此同一短语出现在其他页面时仍可同步；提示页内仍有多个候选时不会猜测。没有页码提示时
  必须在 PDF 全文中唯一匹配。PDF 文本层在换行处保留的断词连字符，以及 OCR 在关系运算符、
  带符号数字和单位周围引入的空格会自动兼容；MinerU 输出的 LaTeX `\\pm` 与 PDF 中的 `±`
  也会统一匹配。找不到文字、存在无法消除的多个候选、本地索引
  不可用，或已打开阅读器的回退搜索失败时会保留本地高亮，不会猜测位置，也不会阻塞 Markdown
  划词交互。页码提示只用于缩小文本匹配范围，source-map 区域坐标不会直接用作新标注的高亮
  矩形。
- 扫描版 PDF 如果没有可提取的文字层，PDF.js 无法生成文字矩形。即使 MinerU 能通过 OCR
  生成 Markdown，这类文字当前仍不能自动同步成精确 PDF 高亮。
...
- `build/mktero-0.2.7.xpi`：可安装插件包
- `build/mktero-0.2.7.xpi.sha256`：只引用 XPI 文件名的 SHA-256 校验文件
...
src/cache/         Markdown、图片与 PDF 文本索引缓存
...
src/pdf/           PDF.js 文本提取与 PDF 标注坐标定位
