...
- 在 Markdown 正文和渲染后的表格、图题等内容中显示 Zotero PDF 高亮与下划线标注，
  ...
  激活图标可继续修改评论。
- 在普通 Markdown 正文中拖动划词后显示紧凑操作条，可选择颜色创建本地高亮，或直接
  创建高亮并添加笔记；本地高亮同样支持点击新增/修改笔记、悬停改色和删除。
...
4. 在只读视图中选择和复制文本，使用 `Ctrl+F` 或 `Cmd+F` 搜索，并通过目录、引用预览
   ...
   普通 Markdown 文本后，可从选区旁的操作条选择高亮颜色或直接添加笔记。
...
在 Markdown 中新建的高亮和笔记独立保存在当前 Zotero 配置文件的
`mktero-annotations/v1` 目录中，未加密、不会发送给 MinerU，也不会通过 Zotero 同步；
清空 Markdown 转换缓存不会删除这些标注。重新解析后仅在划词原文能够安全定位时恢复
高亮，存在多个候选位置时不会猜测。

## 安全边界与当前限制

- Markdown 内容仍是只读视图，不支持编辑、保存或导出；Zotero PDF 标注操作写入当前
  Zotero 文库，Markdown 本地标注操作写入独立的本地标注目录。
- 当前只显示具有划词文本的 Zotero 高亮和下划线标注；独立便签、图片、区域和手写标注
  不会显示。MinerU 的 OCR、断行或重排可能使少量划词无法可靠定位；存在多个候选位置时
  插件不会猜测或错误高亮。
- 新建 Markdown 本地标注时，选区需要位于普通正文或同一个已渲染内容块内；跨越代码块、
