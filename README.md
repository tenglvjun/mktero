...
- 从 Markdown 新建标注时会先立即保存本地高亮。对应 PDF 已打开时会同步 Zotero PDF
  ...
  保留的断词连字符，以及 OCR 在关系运算符、带符号数字和单位周围引入的空格，会在相关
  页面文本可用后自动兼容，无需等待剩余页面提取完成；找不到文字、存在无法消除的多个候选、
  读取器初始化或文本搜索超时时会保留本地高亮，不会猜测位置，也不会阻塞 Markdown 划词
  交互。页码提示只用于缩小 Zotero 原生文本搜索结果，source-map 区域坐标不会直接用作新
  标注的高亮矩形。
