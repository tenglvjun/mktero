...
- 在 Markdown 正文和渲染后的表格、图题等内容中显示 Zotero PDF 高亮与下划线标注，
  ...
  激活图标可继续修改评论。
- 在普通 Markdown 正文中拖动划词后显示紧凑操作条，可选择颜色创建 Zotero PDF 高亮，
  或直接创建高亮并添加笔记；随后在 Markdown 或 PDF 中改色、修改笔记和删除都会同步，
  已打开的 Markdown 标签页会自动刷新 PDF 端的标注变化。
- 在阅读器右侧笔记面板中按文档顺序显示划词、评论、颜色和 PDF 页码；点击已定位的
  笔记可直接跳转到 Markdown 对应位置。右侧笔记面板与左侧目录均可拖动调宽、通过按钮
  或双击分隔线展开和收起。
...
4. 在只读视图中选择和复制文本，使用 `Ctrl+F` 或 `Cmd+F` 搜索，并通过目录、引用预览
   ...
   普通 Markdown 文本后，可从选区旁的操作条选择高亮颜色或直接添加笔记；Mktero 会在
   PDF 中可靠定位这段文字并创建对应的 Zotero 标注。
...
在 Markdown 中新建的高亮和笔记会写入对应 PDF 附件的 Zotero 标注，并由 Zotero 负责
本地保存与同步；内容不会发送给 MinerU。旧版本保存在当前 Zotero 配置文件
`mktero-annotations/v1` 目录中的本地标注，会在再次打开 Markdown 时尝试迁移到 Zotero：
只有原文能在 PDF 中可靠定位时才会迁移，失败的记录会保留在原目录中，不会猜测位置。
...
- 从 Markdown 新建 Zotero 标注时，短文本会等待 PDF 全文完成唯一定位；较长段落在单一
  结果稳定后即可创建，避免大型 PDF 的后续页面扫描长时间阻塞。找不到文字时不会创建，
  PDF 文本层在换行处保留的断词连字符会自动兼容；已发现多个候选时需要选择更长的内容。
  必要时会在后台加载对应 PDF 阅读器以获取页码和高亮矩形。
...
- `build/mktero-0.2.0.xpi`：可安装插件包
- `build/mktero-0.2.0.xpi.sha256`：只引用 XPI 文件名的 SHA-256 校验文件
