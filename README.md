...
- 在普通 Markdown 正文中拖动划词后显示紧凑操作条，可选择颜色立即保存 Markdown 高亮，
  或直接添加笔记；PDF 定位和 Zotero 标注创建随后在后台进行，不会阻塞 Markdown 交互。
  同步成功后，在 Markdown 或 PDF 中改色、修改笔记和删除都会同步，已打开的 Markdown
  标签页会自动刷新 PDF 端的标注变化。
...
4. 在只读视图中选择和复制文本，使用 `Ctrl+F` 或 `Cmd+F` 搜索，并通过目录、引用预览
   ...
   高亮会先立即保存并显示，Mktero 随后在后台定位 PDF 文字并创建对应的 Zotero 标注。
...
`mktero-annotations/v1` 目录中，再在后台写入对应 PDF 附件的 Zotero 标注。同步成功后
由 Zotero 负责本地保存与同步，并移除临时的 Mktero 本地记录；内容不会发送给 MinerU。
只有原文能在 PDF 中可靠定位时才会同步，失败的记录会保留在原目录中，不会猜测位置。
