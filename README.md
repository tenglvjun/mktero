...
- 对 MinerU `content_list.json` 中能与 Markdown 唯一匹配的段落、公式、图片和表格保留
  ...
  来源，第一版跳转到其中第一个位置。唯一匹配兼容常见的智能标点、词内连字符及
...
- 在 Markdown 正文和渲染后的表格、图题等内容中显示 Zotero PDF 高亮与下划线标注，
  保留标注颜色；匹配时兼容 PDF 与 MinerU 之间的 Markdown 转义标点、智能引号、连字符、
  数字引用、句末脚注上标、商标上标、统计运算符空格以及常见温度和正负号 LaTeX 格式
  差异；悬停或键盘聚焦划词时可更改颜色或删除对应 Zotero 标注，点击划词可新增或编辑
  评论；带评论的标注会在起始左上角显示笔记图标，点击或用键盘激活图标可继续修改评论。
