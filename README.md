...
- 在 Markdown 正文和渲染后的表格、图题等内容中显示 Zotero PDF 高亮与下划线标注，
  ...
  带评论的标注会在起始左上角显示笔记图标，仅点击或用键盘激活图标后显示评论和 PDF 页码。
- 在阅读器右侧 PDF 笔记面板中按文档顺序显示划词、评论、颜色和页码；点击已定位的
...
Mktero Markdown 缓存。每次转换或从缓存打开文档时都会重新读取标注；改色和删除操作会
直接保存到当前 Zotero 文库。

## 安全边界与当前限制

- Markdown 内容是只读视图，不支持编辑、保存或导出；标注改色和删除会写入 Zotero。
