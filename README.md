...
只有原文能在 PDF 中唯一定位时才会迁移，失败的记录会保留在原目录中，不会猜测位置。
...
- 从 Markdown 新建 Zotero 标注时，所选文字必须能在 PDF 中唯一定位；找不到文字时不会
  创建标注，出现多个候选时需要选择更长的内容。必要时会在后台加载对应 PDF 阅读器以
  获取页码和高亮矩形。
