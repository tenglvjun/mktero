...
for (const [provider, Extractor, currentProfile] of [
    ...
    test(`${provider} opens an older legacy revision under its actual identity without OCR`, async () => {
        let converted = 0;
        const readKeys = [];
        ...
        const extractor = new Extractor({
            ...
            createCacheKey: async (data, { parserProfile }) => parserProfile === currentProfile ? currentKey
                : parserProfile === LEGACY_FIGURE_PROFILES[provider] ? oldKey : previousKey,
        ...
        assert.deepEqual(readKeys, [currentKey, previousKey, oldKey]);
