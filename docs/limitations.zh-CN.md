...
- 文献列表中的 Markdown 列只在 Mktero 确认该 PDF 于当前转换设置下仍有可用本地缓存后显示。准备中的加载动画只在本次会话有效，关闭扩展后消失。它不会扫描整个文库，不修改条目，也不会同步。清空缓存、缓存过期或解析配置变化后，完成标记会熄灭。此功能出现之前转换过的条目，要等到该缓存被再次打开或写入后才会显示。批量准备会在去重后加入全部选中的 PDF，并且同时转换 3 篇。
