...
选中多条文献或 PDF 附件，选择 `准备所选 Markdown`；右键一个分类，选择 `准备此分类的 Markdown`。这两项都会开始解析，不打开阅读标签，最多排队 50 篇。分类操作只包含该分类自己的条目，不包含只属于子分类的条目。右键单篇仍然是阅读。已有缓存或已在准备中的会跳过。准备过程不会打开阅读标签。Zotero 进度窗口列出每一篇；最小化后队列继续，取消会停止尚未完成的篇目。打开其中一篇会接入已有准备，而不是重新上传。关闭正在转换的标签页不会取消转换，它会转入后台队列。
