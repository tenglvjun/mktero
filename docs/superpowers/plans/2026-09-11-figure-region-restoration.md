...
上方两个 superpowers 执行技能未在本次会话的可用技能中列出。实施使用现有 implement、tdd、diagnosing-bugs 和 code-review 工作流推进；具体完成情况与证据见下文实施状态。

---

## 三步总览

本方案把三步作为一次完整功能开发：一个分支、按依赖连续实施、一个 PR 交付。完成条件包括转换、阅读和持久化的全部通路。

| 阶段 | 解决的问题 | 实现结果 | 对应任务 |
| --- | --- | --- | --- |
| 第一步：修复现有断组 | OCR 清理后多余空行使同一 Figure 丢失部分子图 | 合法布局标记能跨空行收齐成员，独立正文、图注和代码仍构成边界 | T01 |
| 第二步：建立完整 Figure 对象 | 多张图片、图内文字与公共图注没有统一归属 | 两家 OCR 输出统一为 FigureInput；通过父子关系、坐标和精确源范围建立候选及可追溯身份 | T02–T06、T08 |
| 第三步：还原并贯通使用 | 网格猜测无法保留缺角、跨行和不规则布局 | 从本地 PDF 渲染完整图区；成功后原子替换，阅读、引用、翻译、缓存、修订、快照和导出使用同一结果 | T07–T15 |

关键顺序固定为：保留服务商证据 → 绑定源范围 → 对齐本地 PDF 坐标 → 判断归属 → 渲染并校验 PNG → 原子替换 → 正文归一化 → 绑定最终 figureMap → 缓存与发布。图内 OCR 文字只在成功事务中移出正文；图注保持可选择和可翻译。缺少证据或渲染失败的候选保留原内容。

以“16 张子图＋重复 Accuracy 轴标签＋一个 Figure 3 图注”为例：第一步保证合法旧图组不因空行漏成员；第二步把 16 张图片、属于图内的各个 Accuracy 实例和公共图注归为同一个 Figure，图外正文中的 Accuracy 保持独立；第三步从原 PDF 渲染完整图区，正文显示一张保留 4×4 原貌的 PNG 和一次可选中的图注。改窄窗口只缩放整图；引用、翻译和导出都使用这张图。缺角、跨行和不规则布局走相同流程，不需要新增一种 Markdown 网格语法。

## 实施状态（2026-09-12）

分支为 `codex/figure-region-plan-2026-09-11`，评审基线为 `663733a`。T01 保留原提交 `ff04ba1`；公共恢复与所有文档通路已提交为 `3eb6bdf`，原生验证收尾为 `09c5f5a`。实现和 Standards/Spec 两轴复审已完成，最终报告见 [图组验收记录](../../figure-region-validation.md)。

| 任务 | 状态 | 已实现与验证 |
| --- | --- | --- |
| T01 | 完成 | 合法布局标记跨 OCR 空行收齐子图，保留正文和代码边界 |
| T02–T06 | 完成 | 公共模型与资源上限、精确源绑定、两家服务商适配、坐标单位校验、归属解析；Mistral 未证实方向时保留原内容 |
...
全回退回归覆盖实际 render 失败、缺少几何及 1,001 页候选限制，贯穿 restore → finalize → 真实文件缓存 → UI。bootstrap 纠错回归覆盖首次建立 revision base、纠错落盘、重开与 Figure 来源按钮。修订存储保留损坏可选元数据的字段存在性，使可读降级后的旧基线仍能继续保存。

PDF.js 6 legacy 在 Firefox 115 缺少 `Promise.withResolvers` 和 `ArrayBuffer.transferToFixedLength`；新增兼容层在主模块和打包 worker 加载前安装。针对性测试读取实际 PDF 的字体与路径操作，原生像素验证检查字体、轴、箭头、标签、共享图例、位图以及裁图四边。

### 实施检查点

- [x] **坐标与回退契约：** 已核对 MinerU 固定版本的单位依据；三种单位、Rotate、CropBox、UserUnit 回归通过；Mistral 未验证方向保持 preserve。
- [x] **全回退摘要：** 有效空 figures map 经缓存进入单次可关闭提示；无效可选 map 独立降级。
- [x] **翻译与修订：** 四个翻译公开入口和 bootstrap 传递当前 figureMap；兼容缓存、失败重试、纠错重开、不可变 base 与写失败回滚回归通过。
...
一个开发分支连续完成以下 15 个任务，在同一个 PR 中交付。尚未实施的任务各自测试后提交；已经交织在工作树中的改动按可独立审查和测试的功能分组提交，保留 T01 原提交，不强行伪造 15 个独立历史提交。三个阶段全部完成后才视为本功能完成。下表是依赖关系，不要求同时运行多个 agent。
...
| 区域 | 文件 | 改动目的 |
| --- | --- | --- |
| 公共图组 | `src/figures/figure-limits.js`、`figure-model.js`、`figure-page-geometry.js`、`figure-source-binding.js`、`figure-region-resolver.js` | 类型、身份、预算、页坐标、源范围和归属 |
| 恢复事务 | `src/figures/figure-transaction.js`、`figure-restoration-service.js`、`figure-finalization.js`、`figure-normalization.js`、`figure-async.js` | 生成图、精确替换、最终绑定、正文边界和取消 |
...
| PDF 与平台 | 新增 `src/pdf/pdfjs-figure-region.js`、`pdfjs-runtime-compat.js`、`src/platform/zotero-figure-canvas.js`；修改 `src/pdf/pdfjs-bootstrap-environment.js`、`pdfjs-page-crop.js` | 高清区域渲染、XHTML canvas、取消及 Firefox 115 兼容 |
...
| 持久化 | `src/figures/figure-map-serialization.js`、`src/cache/markdown-cache.js`、`src/core/markdown-revision-session.js`、`src/platform/zotero-markdown-revision-store.js` | 共用序列化校验、新 metadata、范围平移、回滚 |
...
- **计划步骤 2：运行 `node --test test/figure-model.test.js`，Expected: FAIL，尚无导出实现。**
- **计划步骤 3：实现模型白名单与设计第 10 节的集中上限。** 拒绝非有限、反向、越界 bbox；验证页索引、重复 ID、重叠消费范围、路径遍历/绝对/远程路径、资产引用和坐标 frame。适配器只构造白名单字段；校验器不修改对象；clone 深拷贝数组、bbox 和 provenance。hash 为 null 仅允许内存模型，持久化校验要求 64 位 hex。

页面 unit 只接受 pt/px/pdf-user-unit；缺省单位保留为缺证据的页面，不授权 compose。非空 dpi 仅允许 px 的有限正数。错误使用 `INVALID_FIGURE_GEOMETRY`；补充如下独立输入校验，避免直到 renderer 才发现非法值：
...
for (const [index, patch] of [
    ...
    { unit: 'px', dpi: 0 },
    { unit: 'px', dpi: -72 },
    { unit: 'px', dpi: Number.NaN },
    { unit: 'px', dpi: Number.POSITIVE_INFINITY },
    { unit: 'pt', dpi: 192 },
].entries()) {
    test(`rejects unsupported page geometry case ${index + 1}`, () => {
        const { input } = makeFigureInput();
        const pages = input.pages.map(page => ({ ...page, ...patch }));
        assert.throws(() => validateFigureInput({ ...input, pages }), {
            code: 'INVALID_FIGURE_GEOMETRY',
        });
    });
}
```
...
- **计划步骤 4：验证普通、边界和恶意值，以及 clone 后修改 bbox/fragments 不影响原对象。** Run: `node --test test/figure-model.test.js`，Expected: PASS。把两个新增源码加入 check。
- **计划步骤 5：提交。** `git commit -m "feat: define bounded figure document metadata"`。
...
- **计划步骤 2：Run `node --test test/figure-source-binding.test.js`，Expected: FAIL。**
- **计划步骤 3：依设计 6.4 实现四层绑定。** 从 GFM AST 读取独立图片、段落、代码/表格排除区；按页与锚点建立索引；序列匹配只有唯一完整解时提交。normalize 比较生成逐字符原偏移表，而不是从规范化字符串索引反推范围。
...
- **计划步骤 4：覆盖 CRLF→LF 后范围、Unicode、Markdown 转义、拆行、同路径重复图像、标题与代码块里的假图片、同词正文、跨页同词、达到比较预算；断言 unresolved 从不被候选消费。** Run: `node --test test/figure-source-binding.test.js test/markdown-source-map.test.js`，Expected: PASS。新增源码加入 check。
- **计划步骤 5：提交。** `git commit -m "feat: bind figure members to exact Markdown ranges"`。
...
**Interfaces:** ZIP 提取结果增加可选 `detailedLayout: { schema: 'mineru-middle-v1', backend, version, pdfInfo: [] }`，仅含白名单版本、页、块和文本字段；`decodeMinerUFigureInput(result, options = {}) -> FigureInput` 将其与稳定 contentList、assets 和原 Markdown 对齐。缺少 detailedLayout 是正常输入。

- **计划步骤 1：写 body 与 caption 分离、父块多图片的 fixture。** 原始 middle JSON 最小结构固定为：
...
- **计划步骤 2：Run `node --test test/mineru-figure-layout-adapter.test.js test/zip-markdown.test.js`，Expected: FAIL，新结构未读取。**
- **计划步骤 3：实现同逻辑根下唯一 middle 文件选择、解压前后预算、有限深度遍历和白名单适配。** 根据实际存在的 assets 解析 span image_path，不能简单字符串拼接后信任路径；遍历全部 body/span，不只取第一个。
...
这一变换只用于 middle 的 PDF 用户空间长度。稳定 contentList 保持原 0–1000；没有本体证据的 bboxKind 标为 group/unknown。

根级 backend/version 与 `MINERU_FIGURE_LAYOUT_OPTIONS` 的已核验集合匹配才允许设置已知 frame。字段缺失、pipeline backend 或未来版本均保留可读结果，页面标记 unsupported-layout-schema。设计第 7 节已记录固定上游 page.get_size() 与 pypdfium2 的单位依据：page_size 不含 UserUnit，声明为 pdf-user-unit，尺寸比较时乘一次。保留该声明，继续验证实际版本的可见框与方向；变更支持集合或几何策略时更新 parser profile 常量。

- **计划步骤 4：测试没有 middle、多个 middle、其他结果根的同名文件、损坏 JSON、声明/实际超限、过深嵌套、路径穿越、span 资产缺失、caption 内恶意 HTML。** 可选布局失败时 full.md/图片仍可提取；日志/警告不含 raw JSON。Run 上述两文件，Expected: PASS；新增源码加入 check。
- **计划步骤 5：提交。** `git commit -m "feat: preserve MinerU figure body and caption geometry"`。

## T05：Mistral 解析拆分并保留图内证据

**Files:** 修改 `src/mistral/mistral-result.js`、`src/mistral/markdown-normalizer.js`、`src/mistral/parser-profile.js`；扩展 `test/mistral-result.test.js`、`test/mistral-markdown-normalizer.test.js`；新增 `test/mistral-figure-input.test.js`；坐标证据记录在 `docs/figure-region-validation.md`。
...
**接续约束：** 保留图内证据是 decode 的默认契约，不依赖调用者额外设置 `preserveFigureText: true`。核对所有入口，确保没有残留 `preserveFigureText`/`legacyLayout` 分支令新转换提前删文或按数量猜列。旧布局标记仍由原 Markdown 读取器兼容。无效或冲突的 bbox 保持 unknown，所有后续几何判断先验证输入，不对 null bbox 运算。

已知 model 只授权读取其字段，不能自动授权 display-cropbox。按设计第 7 节的证据门槛核对坐标：未证实时 coordinateFrame=unknown，已知模型的原因记 missing-geometry；未知模型使用 unsupported-layout-schema。对照 fixture 使用带不对称角标的自有 PDF，保留实际响应的 model、dimensions、images、blocks 与 Markdown 页关系，去掉认证、上传地址等无关字段；将真实 bbox 叠加到本地渲染上检验，不按预期几何手工补写 bbox。

- **计划步骤 1：新增保留原始 OCR 证据的测试。**

```js
test('keeps image interior OCR text available until a crop succeeds', () => {
    const input = decodeMistralResult({ model: 'mistral-ocr-4-1', pages: [{
        index: 0,
...
- **计划步骤 2：Run `node --test test/mistral-figure-input.test.js`，Expected: FAIL。**
- **计划步骤 3：拆出 decode，复用现有 Base64、路径、tables、坐标校验。** blocks 和 images 的同一资产若 bbox 冲突则标记 missing/ambiguous geometry；不凭一个坐标覆盖另一个。取消独立 removeImageInteriorText 调用及提前丢弃 interiorTextRecords；新输入不再 allowFallback 按数量猜列。
...
- **计划步骤 4：更新旧“纯 normalizer 删除图内文字”的测试，验证无渲染能力时保留文字；它原来的“成功后没有重复正文”验收移到 T09 的真实恢复服务链路。** 补充已知 model 但未证实 frame、相同长宽比的 180 度页、旋转方形页、非零 CropBox、尺寸矛盾与未知 model；无证据输入必须保留原 Markdown 和全部 OCR 块。若已取得 frame 契约，明确记录其覆盖组合与回归来源。保留全部既有安全、页序、table、正文/页眉回归。Run: `node --test test/mistral-figure-input.test.js test/mistral-result.test.js test/mistral-markdown-normalizer.test.js`，Expected: PASS。
- **计划步骤 5：提交。** `git commit -m "refactor: separate Mistral decoding from figure restoration"`。
...
- **计划步骤 2：Run `node --test test/figure-region-resolver.test.js`，Expected: FAIL。**
- **计划步骤 3：实现设计第 6 节算法，按明确 parent → 同页几何分量＋唯一 caption → 文字归属 → 外来内容阻挡 → 完整覆盖/范围校验的顺序执行。** 每个 block 最多属于一个 compose 候选；不能用平方根、列数或 Markdown 空行推定原版式。
...
- **计划步骤 4：覆盖 2×2/4×4/缺角/左高右上下/主图跨列/不规则尺寸、上/下图注、单个完整图、重复标签、间隙图例、图外同词、长正文阻挡、跨栏、跨页、无坐标、图注与图区相交和预算边界。** 使用交换 block 输入顺序仍得到相同成员/几何的测试验证确定性；无稳定来源序号时 preserve 而非依赖数组偶然次序。Run: `node --test test/figure-region-resolver.test.js test/figure-source-binding.test.js`，Expected: PASS；加入 check。
- **计划步骤 5：提交。** `git commit -m "feat: resolve complete figures before prose reflow"`。

## T07：独立 PDF.js 整区渲染器

**Files:** 新增 `src/pdf/pdfjs-figure-region.js`、`src/pdf/pdfjs-runtime-compat.js`、`src/platform/zotero-figure-canvas.js`、`src/figures/figure-page-geometry.js`、`src/figures/figure-async.js`、`test/figure-page-geometry.test.js`、`test/pdfjs-figure-region.test.js`、`test/pdfjs-runtime-compat.test.js`、`test/zotero-figure-canvas.test.js`；修改 `src/pdf/pdfjs-bootstrap-environment.js`、`src/pdf/pdfjs-page-crop.js`、`src/figures/figure-model.js`、`test/figure-model.test.js`、`test/pdfjs-bootstrap-environment.test.js`、`scripts/build.mjs`、`package.json`。原 `pdfjs-page-crop.js` 缩略图接口语义保持原状。

**Interfaces:** `createPDFFigureRegionRenderer({ loadDocument, createCanvas, encodePNG, workerSrc, cMapUrl, standardFontDataUrl, wasmUrl })` 返回 `{ open(fileData, options), disposeAll() }`；session 提供 `getPageGeometry(pageIndex, options)`、`renderRegion(request, options)`、`close()`。platform factory 返回 ownerDocument/createCanvas/encodePNG，均可注入测试。getPageGeometry 返回设计第 8 节完整 PageGeometry（包含 viewBox、mediaBox、viewportTransform、userUnit、显示尺寸与 rotation）；纯函数 alignFigureInputToPDF(input, pageGeometryByIndex, options = {}) 位于 figure-page-geometry.js，由 figure-model.js 重导出，失败页写明稳定原因而不猜坐标。figure-async.js 提供设计第 13 节的取消作用域和等待接口，供 renderer 与 T09 共用。

- **计划步骤 1：写不依赖真实 canvas 的区域几何测试。** 这条用例检验分配尺寸和渲染平移；真实像素验收在 T15 独立完成。
...
- **计划步骤 2：Run `node --test test/pdfjs-figure-region.test.js test/zotero-figure-canvas.test.js`，Expected: FAIL。**
- **计划步骤 3：实现 viewport 坐标变换、裁区 canvas、PNG 编码和全局串行 render 队列。** 从 pdfjs-page-crop.js 把 createMkteroCanvasFactory、MkteroFilterFactory 原样提取到 pdfjs-bootstrap-environment.js 并导出，旧缩略图与新入口共同导入；保持 CanvasFactory/FilterFactory、worker、CMap、字体、WASM 和禁用远程 fetch 的加载选项。向 worker 传入有界 PDF 字节副本，不能 detach 转换原始 fileData。 padding 已由 resolver 确定，renderer 不再次外扩；渲染请求的 bbox 是最终边界。
...
UserUnit 的回归用三种等价输入，防止把 PDF.js 已放大的显示尺寸再次乘二。这验证公共变换契约；MinerU 的字段单位已有 T04 所引上游依据，真实渲染仍须通过 T15：

```js
test('aligns equivalent page units without applying UserUnit twice', () => {
    const { input } = makeFigureInput();
    const geometry = {
        pageIndex: 0, width: 1200, height: 1600, rotation: 0, userUnit: 2,
        coordinateFrame: 'display-cropbox',
        viewBox: [0, 0, 600, 800], mediaBox: null,
        viewportTransform: [2, 0, 0, -2, 0, 1600],
    };
    for (const dimensions of [
        { width: 600, height: 800, unit: 'pdf-user-unit', dpi: null },
        { width: 1200, height: 1600, unit: 'pt', dpi: null },
        { width: 2400, height: 3200, unit: 'px', dpi: 144 },
    ]) {
        const pages = [{ ...input.pages[0], ...dimensions }];
        const aligned = alignFigureInputToPDF({ ...input, pages }, [geometry]);
        assert.equal(aligned.pages[0].geometryReason, undefined);
        assert.equal(aligned.pages[0].width, 1200);
        assert.equal(aligned.pages[0].height, 1600);
        assert.deepEqual(aligned.blocks.map(block => block.bbox),
            input.blocks.map(block => block.bbox));
    }
});
```

- **计划步骤 4：覆盖 0/90/180/270 度、MediaBox/CropBox 非零偏移、裁区触边、极端长宽、未知 frame、尺寸冲突、超大 PDF、超时、排队取消、渲染取消、编码期间取消、disposeAll 幂等、两文档共用渲染队列。** 检查 canvas 分配预算、renderTask.cancel、page.cleanup、document/loadingTask.destroy 和 listener 数量。Run 上述文件及 `test/figure-page-geometry.test.js`、`test/pdfjs-page-crop.test.js`、`test/pdfjs-bootstrap-environment.test.js`、`test/figure-model.test.js`、`test/zotero-source-peek.test.js`，Expected: PASS；新源码加入 check。
- **计划步骤 5：提交。** `git commit -m "feat: render bounded figure regions from local PDFs"`。

## T08：图组替换事务与最终元数据

**Files:** 新增 `src/figures/figure-transaction.js`、`src/figures/figure-finalization.js`、`src/figures/figure-normalization.js`、`test/helpers/restored-figure-fixture.js`、`test/figure-transaction.test.js`、`test/figure-finalization.test.js`；修改 `src/mineru/mineru-result.js`、`src/mineru/figure-panel-normalizer.js`、`src/mineru/markdown-normalizer.js`、`src/markdown/figure-layout-normalizer.js`、`src/core/markdown-source-map.js`、`package.json`。

**Interfaces:** `composeFigureDraft(input, completed, options = {}) -> FigureDraft`；completed 每项为 `{ candidate, crop, assetPath }`。`finalizeFigureMap(prepared, blueprints, { hash, signal, preserved = [] }) -> Promise<ConvertedDocument>`。额外导出 `finalizeRestoredDocument(input, draft, { prepare, hash, signal })`，将 prepare 和最终绑定包成一个可回退操作；prepare 是注入的 provider 纯归一化函数。blueprint 含 renderCaption，用于验证最终图片描述与事务写入的已转义 Markdown 一致。

- **计划步骤 1：先证明消费范围不会吞正文。** fixture 在第一、二张图之间插入 role=body 的图外段落，保留其 bbox 在图区外；使用明确父关系候选和 renderer stub 的成功 crop。测试通过真实 compose/finalize 路径。
...
- **计划步骤 2：Run `node --test test/figure-transaction.test.js test/figure-finalization.test.js`，Expected: FAIL。**
- **计划步骤 3：实现事务。** 先验证所有源范围及 metadata/资产预算，再按 from 排序构造非重叠编辑，从后向前应用；图注范围替换成一个转义后的规范图片块，其余成员逐项移除。只吸收被删除块自己的空白边界，不能全篇重排。input/contentList/assets 全部按新对象生成；移除已消费 blocks，把未消费 block.sourceRanges 和页 markdownRange 按同一 edits 平移，加入新 image 块，避免后续 prepare 读到旧范围或旧文本。
...
- **计划步骤 4：验证多图事务、前/后图注、无图注明确父图、同名资产冲突、公式/引用/转义图注、图片缺失、追溯超限、相交范围、恶意源文本、源对象未被修改、最终锚点缺失与幂等。** finalizeRestoredDocument 把 draft.preserved 合入最终 map；全保留但有诊断时写 figures=[] 的有效 map。只在图组最终绑定失败时回到原 input 重新 prepare，并把放弃的候选记为 ambiguous-source-range，不保留生成资产；AbortError 必须继续抛出。Run 上述文件加 `test/mineru-result.test.js test/markdown-source-map.test.js`，Expected: PASS；新增源码加入 check。

增加以下全回退契约回归，使用测试文件已有 hash 和 helper。它断言正常有效的空 figures map；与“可选 metadata 无效时返回 null”用例并存：

```js
test('finalizes valid preserve metadata when no figure was composed', async () => {
    const { input } = makeFigureInput();
    const preserved = [{ id: 'fig-p0-b4', pageIndex: 0, reason: 'render-failed' }];
    const document = await finalizeRestoredDocument(input, {
        input, blueprints: [], preserved,
    }, { prepare: value => value, hash });
    assert.equal(document.markdown, input.markdown);
    assert.deepEqual(document.assets, input.assets);
    assert.deepEqual(document.figureMap.figures, []);
    assert.deepEqual(document.figureMap.preserved, preserved);
    assert.equal(document.figureMap.markdownHash,
        await hash(new TextEncoder().encode(input.markdown)));
});
```

T09 再用实际 restore 返回的失败 draft 调用 finalize；T10 将该结果写缓存再读出；T12 验证重开仍提示一次。仅对 helper 手写 draft 的成功测试不完成这三个集成验收。有效摘要不得因 blueprints 为空直接丢弃；无效 map 则按设计保留文档并降级，不能绕过校验。

- **计划步骤 5：提交。** `git commit -m "feat: commit restored figures with source provenance"`。
...
- **计划步骤 1：把上一轮“2×2＋图内文字”放进完整服务链路。** renderer stub 对应真实坐标返回有界 PNG；断言四成员进入一个 sourceId、正文没有 axisLabel、bodyText 保留、caption 只出现一次、sourceMap 有正确 visualBBox/captionBBox。
...
- **计划步骤 2：Run `node --test test/figure-restoration-service.test.js test/mineru-conversion.test.js test/mistral-conversion.test.js`，Expected: FAIL，新服务/注入不存在。**
...
- **计划步骤 4：验证 cold/cache/resumed/no-key/forceRefresh、两 provider 隔离、关闭标签页、两窗口同 PDF、转换代际竞争、编码结束前取消、缓存提交前取消、恢复超时和缓存写失败。** 在 MinerU commit 前复查 signal 与当前 batchID，取消不能删除尚可恢复的 pending task；旧成功标签页保持其原内容。在 `test/bootstrap-abort-controller.test.js`、`test/markdown-tab-presenter.test.js`、`test/markdown-window.test.js` 中追加对应生命周期断言。Run: `node --test test/figure-restoration-service.test.js test/mineru-conversion.test.js test/mistral-conversion.test.js test/markdown-document-service.test.js test/mineru-extractor.test.js test/mistral-extractor.test.js test/markdown-loading-state.test.js test/bootstrap-abort-controller.test.js test/markdown-tab-presenter.test.js test/markdown-window.test.js`，Expected: PASS。
- **计划步骤 5：提交。** `git commit -m "feat: restore figures before conversion cache commits"`。

## T10：figureMap 缓存与解析身份

**Files:** 新增 `src/figures/figure-map-serialization.js`；修改 `src/cache/markdown-cache.js`、`src/mineru/parser-profile.js`、`src/mistral/parser-profile.js`、`package.json`；扩展 `test/markdown-cache.test.js`、`test/mineru-result.test.js`、`test/mistral-conversion.test.js`。
...
共用 `serializeFigureMap(map, document, { hash, maxBytes }) -> Promise<{ map, json, bytes } | null>` 与 `parseFigureMapJSON(json, document, { hash, maxBytes }) -> Promise<FigureMap>`，分别用于生成持久化 hash/JSON 和有界读取校验。后续修订与快照直接复用；无 map 的序列化结果为 null，非法 map 抛错由对应存储边界决定降级或回滚。

- **计划步骤 1：在现有内存 IO cache fixture 中加入完整往返。** fixture 的 result 由 T08/T09 真实 finalize 构建，避免手写不匹配 hash 的模型。
...
- **计划步骤 2：Run `node --test test/markdown-cache.test.js`，Expected: FAIL，figureMap 被丢失。**
- **计划步骤 3：按 sourceMap/chromeRanges 的代际文件模式增加读写、校验、统计、扫描和清理。** 写 metadata 之前 figureMap hash 与 Markdown 匹配；临时文件失败清理不能删除上一代。读取未知/损坏可选 map 时忽略 map 并返回 Markdown/资产。
...
- **计划步骤 4：覆盖缺文件、大小/hash 不符、未知版本、恶意路径、无效范围、PNG 引用缺失、8 MiB 边界、写入失败、并发 put/get、旧 metadata、clear、expiry 与总量淘汰；验证两家新 key 与 0.3.9 key 不同。** Run: `node --test test/markdown-cache.test.js test/mineru-result.test.js test/mistral-conversion.test.js`，Expected: PASS。
- **计划步骤 5：提交。** `git commit -m "feat: persist versioned figure maps with cached documents"`。
...
- **计划步骤 2：Run `node --test test/figure-map-transforms.test.js test/markdown-revision-session.test.js test/zotero-markdown-revision-store.test.js`，Expected: FAIL，元数据未传递。**
- **计划步骤 3：实现深拷贝、偏移变换和可选持久化。** 与原 sourceMap 使用相同端点关联方向；纯函数不计算异步 hash，store.save 前计算并校验；不可变 revision base 的 figureMap 只写一次。旧 base 缺字段时取 null，不修改已有 base 文件。

新写入 metadata 的 `figureMapHash` 是 JSON 原字节 SHA-256；map 内 `markdownHash` 绑定正文。兼容旧 metadata 缺少 figureMapHash 的读取，缺少新增字段不能让既有修订打不开；若保存需要补齐 hash，只计算既有 base 文件的真实值。已有 base 的图组身份、坐标、原片段或资产与新输入不一致时拒绝替换。`#writeImmutableBase()` 每成功创建一个文件就登记其所有权，失败时按登记清理；任何预先存在的文件和之前已发布 metadata 都不能删除。

在现有 revision-store harness 中覆盖：无 map 的旧格式、有 map 但无新增 hash、hash 不符、不可变 map 被修改、首次保存各写入阶段失败、metadata 提交失败、旧基线下保存失败。每个失败用例比较失败前后用户文件和旧 base 的字节，以及 load 结果；首次保存失败后再保存一次也应成功，不能遗留阻断重试的半成品。范围编辑仍由 revision session 作用于工作副本，不重写不可变 base。
...
- **计划步骤 4：验证旧 profile 命中时 OCR 调用数和 render 调用数均为 0、结果 identity 仍为旧值、forceRefresh 跳过兼容查询、缓存关闭但已有修订仍可读、正文纠错/撤销后 Figure 引用与 PDF 位置一致。** Run 上述文件和 `test/mineru-extractor.test.js test/mistral-extractor.test.js`，Expected: PASS；两新源码加入 check。
- **计划步骤 5：提交。** `git commit -m "feat: preserve figure metadata across revisions and profile upgrades"`。
...
- **计划步骤 2：Run `node --test test/figure-analysis.test.js test/markdown-figure-references.test.js test/markdown-asset-outline.test.js`，Expected: FAIL。**
- **计划步骤 3：按 figureMap 先生成明确 FigureView，再合并不与其范围相交的 legacy 分析结果。** 不让 legacy 分组把已合成 PNG 吸收到下一幅图。FigureView 带入经过验证的 panels；子图引用只在标签唯一时取 panel bbox，sourceId 仍为父图，预览保留完整 PNG。使用当前图片节点里的 caption，而不是 metadata 内的旧文字；验证 assetPath、范围和 caption label。旧无 map 文档继续走现有分析。
...
- **计划步骤 5：提交。** `git commit -m "feat: share restored figure identities across reader surfaces"`。

## T13：翻译与双语中的同一 Figure

**Files:** 修改 `src/markdown/markdown-translation-blocks.js`、`src/editor/translation-presentation.js`、`src/ai/markdown-translation-service.js`、`src/figures/figure-analysis.js`、`src/ui/markdown-window.js`、`src/bootstrap.js`；扩展 `test/markdown-translation-blocks.test.js`、`test/markdown-translation-service.test.js`、`test/figure-analysis.test.js`、`test/inline-markdown-editor.test.js`、`test/bootstrap-abort-controller.test.js`、`test/markdown-window.test.js`。
...
四个现有公开入口 `translateDocument(options)`、`getCachedDocumentTranslation(options)`、`listCachedDocumentTranslationVariants(options)`、`reconcileDocumentTranslation(options)` 的 options 均增加可选 figureMap，缺省 null；其他参数与返回字段不变。内部 `buildDocumentTranslationViews(source, blocks, translations, { figureMap = null } = {})` 必须转发第四参数。新增参数仅参与本地视图重建，不进入 gateway 请求或单独改变翻译文本缓存键。

- **计划步骤 1：在现有翻译测试文件加入完整图图注翻译回归。**
...
在 `test/markdown-translation-service.test.js` 同时加入公开服务调用的回归，复用该文件现有 SETTINGS 和 translateBatchRequest，导入 T08 的 makeRestoredFigureDocument。以下用例覆盖 fresh、精确缓存、变体枚举和重建；即使辅助函数测试已通过，服务漏传 map 时仍应失败：

```js
test('preserves figure identity through public translation and cache APIs', async () => {
    const { markdown, figureMap } = await makeRestoredFigureDocument();
    let cached = null;
    let requestCount = 0;
    const service = new MarkdownTranslationService({
        ...
        cache: {
            ...
            putTranslation: async (_documentKey, _translationKey, value) => {
                cached = value;
            },
        },
        aiGateway: {
            async generateText(request) {
                requestCount++;
                assert.ok(!JSON.stringify(request.messages)
                    .includes('Time since treatment (days)'));
                return { text: translateBatchRequest(request.messages[1].content,
                    source => source.replace('Treatment response in four panels.',
                        '四个子图的处理结果。')) };
            },
        },
    });
    const options = { documentKey: 'a'.repeat(64), markdown, figureMap,
        targetLanguage: 'zh-CN' };
    const fresh = await service.translateDocument(options);
    assert.ok(requestCount > 0);
    const completedRequests = requestCount;
    const reopened = await service.getCachedDocumentTranslation(options);
    ...
    const reconciled = await service.reconcileDocumentTranslation({
        ...options, existingTranslation: fresh,
    });
    assert.equal(variants.length, 1);
    for (const result of [fresh, reopened, variants[0], reconciled]) {
        assert.ok(result);
        assert.equal(result.translatedFigureViews.length, 1);
        assert.equal(result.comparisonFigureViews.length, 1);
        assert.equal(result.translatedFigureViews[0].sourceId, figureMap.figures[0].id);
        assert.equal(result.comparisonFigureViews[0].sourceId, figureMap.figures[0].id);
        assert.equal(result.comparisonMarkdown
            .split(figureMap.figures[0].render.assetPath).length - 1, 1);
    }
    assert.equal(requestCount, completedRequests);
});
```

- **计划步骤 2：Run `node --test test/markdown-translation-blocks.test.js test/markdown-translation-service.test.js test/figure-analysis.test.js`，先确认缺失契约产生失败。** 已有辅助视图可能通过；新增公开服务用例须能检测 sourceId 丢失，不能以已存在字段就跳过。
- **计划步骤 3：沿已有 protected fragments 保护图片路径，description 走现有可翻译通路。** 保留 removeRepeatedComparisonImages 的去重行为。合并 blockRanges 后先用 sourceFrom/sourceTo 定位原 Figure 所属块，再按 viewKind 选择 translatedFrom/To 或 comparisonSourceFrom/To 窗口，最后核对实际图片 AST 节点。
...
| 调用路径 | 必须传递的本地图组上下文 |
| --- | --- |
| bootstrap 首次翻译、失败重试与强制重译 | 同次捕获的 markdown、chromeRanges 和 figureMap |
| bootstrap 读取缓存、枚举语言变体、纠错后重建 | 当前修订对应的 markdown 和 figureMap |
| #readCachedDocumentTranslation / #readCompatibleDocumentTranslation / #getCachedDocumentTranslationForSettings | 显式接收当前 figureMap，向视图构造转发 |
| normalizeVisibleTranslation | 接收当前 figureMap，用重建后的 blockRanges 生成当前视图 |
| buildDocumentTranslationViews 的全部调用点 | 第四参数 `{ figureMap }`，包括新请求完成分支 |

四个私有/辅助路径原有参数后增加 `figureMap = null`，相邻调用点一起更新；纯视图构造使用上文的 options 参数。不要从旧译文结果取 map 代替当前源文档。bootstrap 开始异步工作时捕获不可变输入；提交时沿现有请求跟踪和文档版本检查拒绝过期结果。

```js
function buildDocumentTranslationViews(source, blocks, translations, { figureMap = null } = {}) {
    return createDocumentTranslationViews(source, blocks, translations, { figureMap });
}
```

阅读窗口可以按当前 figureMap/blockRanges 派生 FigureView，但其结果须与服务返回值一致；翻译缓存读取时也重建派生视图。旧缓存没有 FigureView 字段仍可复用块译文，不因仅补 map 再发送翻译请求。
...
- **计划步骤 4：断言 provenance.fragments 不进入 gateway 请求；译文篡改 assetPath 沿现有 protected-content 失败规则拒绝；原/译/双语切换、翻译失败、纠错后重译、翻译缓存恢复均保持 sourceId 和图片完整。** 扩展现有兼容缓存、无缓存、partial/重试和 visible 重建用例：传真实 fixture 的 map，分别断言两种视图 sourceId；延迟 gateway 返回期间更新修订，确认旧结果未发布。bootstrap stub 明确检查四种公开服务调用收到与 markdown 同版的 map，窗口端与服务端分析结果一致。保留现有“不重复图片”测试，不能为满足新 map 修改其预期。Run: `node --test test/markdown-translation-blocks.test.js test/markdown-translation-service.test.js test/figure-analysis.test.js test/inline-markdown-editor.test.js test/bootstrap-abort-controller.test.js test/markdown-window.test.js`，Expected: PASS。
- **计划步骤 5：仅暂存本任务文件并提交。** `git commit -m "feat: retain figure identity in translated reading views"`。
...
- **计划步骤 1：在已有 store 的 createHarness 中测试保存、读取及可选附件丢失。**
...
- **计划步骤 2：Run `node --test test/saved-markdown-note-format.test.js test/zotero-saved-markdown-store.test.js test/saved-markdown-open-resolver.test.js`，Expected: FAIL，新附件及读版本尚未支持。**
- **计划步骤 3：实现可选 metadata 附件的完整生命周期。** 创建阶段计算 Markdown hash，以 persisted=true 校验 map；JSON UTF-8 长度≤8 MiB，另计算 JSON hash。只有三项字段完整有效时才作为可读附加 metadata：
...
- **计划步骤 4：完成导出和快照安全回归。** Markdown 导出仍只传普通 markdown/assets，测试不能让导出器读取 figureMap 或原 PDF：
...
- **计划步骤 1：建立可以重复生成、可人工核对的样例集。** 固定使用自有内容，PDF 同时包含位图 XObject、矢量边框/连线/箭头、轴刻度、子图标签和独立图注。场景表为：
...
- **计划步骤 2：把两个 provider 的完整恢复路径接入同一回归，先确认组合用例能发现错误。**
...
服务商坐标验收与本地渲染验收分别记录。MinerU 引用设计第 7 节的固定源码及单位依据；Mistral 记录实际返回的 model、请求选项、自有 PDF hash 和离线响应 fixture hash。用不对称角标逐一核验旋转、CropBox 及方形页面方向，并保存原始框与显示框的叠加图。没有真实响应或明确 frame 契约时，该组合保持未验收和 preserve；合成 JSON 跑满像素矩阵也不补足这项证据。

当前 harness 的单点面板底色检查仅是第一层。必须另为轴、箭头、标签、图例与嵌入位图建立独立 feature 区域，统计区域内暗像素或预期色块的覆盖；期望区域来自固定绘图元素，不由待测 resolver 生成。检查裁图四边对应的页面坐标，并确认与独立 caption/body 框无交集。像素映射使用实际 crop 尺寸和渲染 scale，不能在触发像素/边长上限后仍写死 192 DPI。每类特征分别记录结果，任一缺失使该 crop 验证失败。
...
- **计划步骤 5：补齐用户说明、本地化和全部交付检查。** 新增阶段/提示使用以下中英文含义，并接入现有文档级 warning 展示通路；同一文档最多一次恢复不完整提示：
...
全部新增 src 文件及五个开发脚本（含独立 reader harness）加入显式 check 清单；不改发布版本。fixture 生成器的 Node canvas 仅为开发依赖，若直接使用 `@napi-rs/canvas`，应明确声明并同步 lockfile，不能依赖偶然的传递依赖；不新增生产依赖。更新打包测试确保 Firefox 115 bundle 成功、既有 PDF 资源可定位，开发 harness/fixtures 不进入 XPI。README 所述行为与可用服务商证据相符，不能宣传所有无坐标 PDF 都能自动还原。
...
shasum -a 256 test/fixtures/figures/compound-figures.pdf test/fixtures/figures/compound-figures.expected.json
node scripts/generate-figure-fixtures.mjs
shasum -a 256 test/fixtures/figures/compound-figures.pdf test/fixtures/figures/compound-figures.expected.json
...
没有 nvm 的环境将 `/Users/tenglvjun/.nvm/versions/node/v24.15.0/bin` 前置到 PATH，不能退回本机默认的 Node 25。两次 SHA-256 输出必须逐项一致；第二次生成不引入新的二进制/JSON 变化。最终 build 之后运行浏览器和 Zotero harness，防止 build 清理报告目录；保存运行版本、最终代码提交或 diff 标识、fixture hash、命令、截图与报告位置。检查未执行项必须进入交付说明，实际 Zotero 矩阵缺项时不得把完整验收标为通过。

- **计划步骤 6：仅暂存本任务产物并提交最终验收记录。** `git commit -m "test: verify complete figure restoration across document workflows"`。最后对基线以来的完整 diff 自审；一个 PR 包含三个阶段和真实验证证据，PR 标题/描述以最终行为为准。此任务不自动发布版本。
...
| A04 失败和取消保留内容 | T07–T12 | 每类 preserve 原因、全回退摘要的缓存/UI 往返、时限/预算、取消前后 cache/pending task 检查 |
...
| A07 所有文档通路及旧数据 | T09–T14 | cache/revision/translation/snapshot/export 往返；四个翻译公开入口及 bootstrap 传递；精确旧 profile fixture |
| A08 PDF 图形与坐标正确 | T04、T05、T07、T15 | 服务商坐标证据；真实 PDF.js/canvas 的线、字、箭头、旋转、CropBox/UserUnit 核验 |
...
- [x] 原/译/双语遵守现有图片去重行为；旧缓存、旧修订、旧快照的真实身份与可读性保持。
- [x] 翻译服务四个公开入口、兼容缓存与失败重试均有 sourceId 回归；旧异步结果不能覆盖新修订。
- [x] 启用自动合成的 MinerU 版本有固定上游依据；Mistral 未证实方向时保留原内容，其真实坐标能力仍未验收。
- [x] 安全、资源预算、原子写入、取消和资源计数检查完成；内存验证限于有界分配与资源生命周期，不宣称已完成 RSS/GC 堆基准。
- [x] 自动验证和真实 Zotero 验证分别有证据，服务商与未检查用户原文件的限制如实记录。
